...
export default function VistoriaPage() {
  ...
  function sigDown(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    ctx.beginPath(); ctx.moveTo(x, y); ctx.strokeStyle = "#fff"; ctx.lineWidth = 2; ctx.lineCap = "round"
  }
  ...
  if (loading) return <div style={{ padding: 40, color: "#71717A", fontSize: 14 }}>Carregando vistoria…</div>

  const card: React.CSSProperties = { background: "#0D0D0D", border: "1px solid #1F1F1F", borderRadius: 14, padding: 18 }
  const label: React.CSSProperties = { fontSize: 12, fontWeight: 700, color: "#A1A1AA", textTransform: "uppercase", letterSpacing: "0.04em", margin: "0 0 12px" }

  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "24px 20px 64px" }}>
      {/* header */}
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
        <button onClick={() => router.back()} style={{ width: 36, height: 36, borderRadius: 9, background: "transparent", border: "1px solid #1F1F1F", color: "#A1A1AA", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><ArrowLeft size={16} /></button>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <ShieldCheck size={22} color="#0066FF" />
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#fff", margin: 0, letterSpacing: "-0.5px" }}>Vistoria de entrada</h1>
        </div>
      </div>
      <p style={{ fontSize: 13, color: "#71717A", margin: "0 0 20px" }}>Registre o estado do carro na chegada — fotos, avarias e assinatura. Vira prova contra disputa.</p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: 10 }}>
            {photoUrls.map((url, i) => (
              <div key={i} style={{ position: "relative", aspectRatio: "4/3", borderRadius: 10, overflow: "hidden", border: "1px solid #1F1F1F" }}>
                <img src={url} alt={`foto ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                {!locked && <button onClick={() => setPhotoUrls((p) => p.filter((_, j) => j !== i))} style={{ position: "absolute", top: 4, right: 4, width: 24, height: 24, borderRadius: 6, background: "rgba(0,0,0,0.7)", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={13} /></button>}
              </div>
            ))}
            {!locked && photoUrls.length < MAX_PHOTOS && (
              <button onClick={() => fileRef.current?.click()} disabled={uploading} style={{ aspectRatio: "4/3", borderRadius: 10, border: "1px dashed #2A2A2A", background: "#0A0A0A", color: "#71717A", cursor: uploading ? "wait" : "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 11, fontFamily: "inherit" }}>
          ...
          {!locked && (
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              {(Object.keys(SEV) as Severity[]).map((s) => (
                <button key={s} onClick={() => setSev(s)} style={{ flex: 1, height: 34, borderRadius: 8, border: `1px solid ${sev === s ? SEV[s].color : "#2A2A2A"}`, background: sev === s ? `${SEV[s].color}1A` : "transparent", color: sev === s ? SEV[s].color : "#A1A1AA", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>{SEV[s].label}</button>
              ))}
            </div>
          )}
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {/* diagrama */}
            <div onClick={addMark} style={{ position: "relative", width: 200, height: 340, flexShrink: 0, background: "#0A0A0A", borderRadius: 16, border: "1px solid #1F1F1F", cursor: locked ? "default" : "crosshair", margin: "0 auto" }}>
              <svg viewBox="0 0 200 340" width="200" height="340" style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
                <rect x="38" y="20" width="124" height="300" rx="48" fill="none" stroke="#2A2A2A" strokeWidth="2" />
                <path d="M58 70 L142 70 L128 110 L72 110 Z" fill="none" stroke="#2A2A2A" strokeWidth="1.5" />
                <path d="M72 230 L128 230 L142 270 L58 270 Z" fill="none" stroke="#2A2A2A" strokeWidth="1.5" />
                <rect x="62" y="120" width="76" height="100" rx="6" fill="none" stroke="#222" strokeWidth="1" />
                <text x="100" y="174" textAnchor="middle" fill="#2A2A2A" fontSize="10">frente ↑</text>
              </svg>
              {marks.map((m, i) => (
                <button key={i} onClick={(e) => { e.stopPropagation(); setSelMark(i) }} style={{ position: "absolute", left: `${m.x * 100}%`, top: `${m.y * 100}%`, transform: "translate(-50%,-50%)", width: 18, height: 18, borderRadius: "50%", background: SEV[m.severity].color, border: selMark === i ? "2px solid #fff" : "2px solid rgba(0,0,0,0.5)", cursor: "pointer", padding: 0, fontSize: 9, fontWeight: 800, color: "#000" }}>{i + 1}</button>
              ))}
            </div>
            {/* lista de marcas */}
            <div style={{ flex: 1, minWidth: 220 }}>
              {marks.length === 0 && <p style={{ fontSize: 13, color: "#52525B" }}>Nenhuma avaria marcada. {locked ? "" : "Escolha a gravidade e toque no diagrama."}</p>}
              {marks.map((m, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", marginBottom: 6, borderRadius: 9, background: selMark === i ? "#161616" : "#0A0A0A", border: `1px solid ${selMark === i ? SEV[m.severity].color : "#1F1F1F"}` }}>
                  ...
                  {locked ? (
                    <span style={{ fontSize: 12, color: "#A1A1AA", flex: 1 }}>{SEV[m.severity].label}{m.note ? ` — ${m.note}` : ""}</span>
                  ) : (
                    <>
                      <select value={m.severity} onChange={(e) => setMarks((arr) => arr.map((x, j) => j === i ? { ...x, severity: e.target.value as Severity } : x))} style={{ height: 28, background: "#0A0A0A", border: "1px solid #2A2A2A", borderRadius: 7, color: "#fff", fontSize: 12, fontFamily: "inherit" }}>
                        {(Object.keys(SEV) as Severity[]).map((s) => <option key={s} value={s}>{SEV[s].label}</option>)}
                      </select>
                      <input value={m.note ?? ""} onChange={(e) => setMarks((arr) => arr.map((x, j) => j === i ? { ...x, note: e.target.value } : x))} placeholder="nota (ex: risco porta)" style={{ flex: 1, height: 28, background: "#0A0A0A", border: "1px solid #2A2A2A", borderRadius: 7, color: "#fff", fontSize: 12, padding: "0 8px", outline: "none", fontFamily: "inherit", minWidth: 80 }} />
        ...
        <div style={card}>
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} disabled={locked} placeholder="Nível de combustível, km, pertences no veículo, estado geral…" rows={3} style={{ width: "100%", background: "#0A0A0A", border: "1px solid #252525", borderRadius: 10, color: "#fff", fontSize: 13, padding: "10px 12px", outline: "none", fontFamily: "inherit", resize: "vertical", boxSizing: "border-box" }} />
        </div>
        ...
        <div style={card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            ...
            {!locked && <button onClick={clearSig} style={{ background: "none", border: "none", color: "#71717A", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Limpar</button>}
          </div>
          {locked && signature ? (
            <img src={signature} alt="assinatura" style={{ width: "100%", maxWidth: 380, height: 120, objectFit: "contain", background: "#0A0A0A", borderRadius: 10, border: "1px solid #1F1F1F" }} />
          ) : (
            <canvas ref={sigRef} width={760} height={160} onPointerDown={sigDown} onPointerMove={sigMove} onPointerUp={sigUp} onPointerLeave={sigUp} style={{ width: "100%", height: 140, background: "#0A0A0A", borderRadius: 10, border: "1px solid #252525", touchAction: "none", cursor: locked ? "default" : "crosshair" }} />
          )}
        </div>
