    ...
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "24px 20px", fontFamily: "'Inter',-apple-system,sans-serif" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: "none", color: "var(--c-text-3)", fontSize: 13, cursor: "pointer", marginBottom: 12, padding: 0, fontFamily: "inherit" }}
        >
          <ArrowLeft size={14} /> Voltar
        </button>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--c-text)", margin: 0, letterSpacing: "-0.02em" }}>
          Repasses · {employee?.name}
        </h1>
        <p style={{ fontSize: 13, color: "var(--c-text-2)", marginTop: 6, lineHeight: 1.6 }}>
          Exceções específicas deste funcionário. Quando não há exceção pra um serviço,
          ele recebe o <strong style={{ color: "var(--c-text)" }}>repasse padrão</strong> definido em cada serviço.
        </p>
      </div>
      ...
      <div style={{ marginBottom: 20, padding: "14px 16px", background: "var(--c-surface-2)", border: "1px solid var(--c-border)", borderRadius: 12 }}>
        ...
        <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 8 }}>
          {services.length === 0 ? (
            <p style={{ fontSize: 13, color: "var(--c-text-4)" }}>Nenhum serviço cadastrado.</p>
          ) : services.map(s => {
            const hasException = rules.some(r => r.serviceId === s.id)
            return (
              <div key={s.id} style={{ padding: "8px 10px", background: "var(--c-bg)", border: "1px solid var(--c-border)", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 12, color: hasException ? "var(--c-text-4)" : "var(--c-text-2)", textDecoration: hasException ? "line-through" : "none" }}>
                  {s.name}
                </span>
                <span style={{ fontSize: 12, fontWeight: 600, color: s.commissionPercent != null ? "#A78BFA" : "var(--c-text-4)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        ...
        {availableServices.length > 0 && (
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, height: 34, padding: "0 14px", borderRadius: 8, background: "linear-gradient(135deg, #0066FF, #7C3AED)", color: "white", border: "none", cursor: "pointer", fontSize: 12.5, fontWeight: 600, fontFamily: "inherit" }}
          >
      ...
      {rules.length === 0 ? (
        <div style={{ padding: "32px 20px", border: "1px dashed var(--c-border)", borderRadius: 12, textAlign: "center" }}>
          <Percent size={28} color="var(--c-text-4)" style={{ margin: "0 auto 8px" }} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {rules.map(rule => {
            ...
            return (
              <div key={rule.id} style={{ padding: "12px 16px", background: "var(--c-surface)", border: "1px solid var(--c-border)", borderRadius: 10, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <>
          ...
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "var(--c-surface)", border: "1px solid var(--c-border)", borderRadius: 16, padding: 24, width: "100%", maxWidth: 440, zIndex: 101, boxShadow: "0 24px 48px rgba(0,0,0,0.6)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: "var(--c-text-2)", display: "block", marginBottom: 6 }}>Serviço</label>
                <select
                  value={selectedServiceId}
                  onChange={(e) => setSelectedServiceId(e.target.value)}
                  style={{ width: "100%", height: 38, padding: "0 10px", background: "var(--c-bg)", border: "1px solid var(--c-border-2)", borderRadius: 8, color: "var(--c-text)", fontSize: 13, outline: "none", fontFamily: "inherit" }}
                >
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: "var(--c-text-2)", display: "block", marginBottom: 6 }}>% deste funcionário neste serviço</label>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 20 }}>
              <button
                ...
                style={{ height: 36, padding: "0 16px", borderRadius: 8, background: "transparent", border: "1px solid var(--c-border-2)", color: "var(--c-text-2)", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}
              >
                Cancelar
              </button>
              <button
                onClick={handleAdd}
                disabled={saving}
                style={{ height: 36, padding: "0 16px", borderRadius: 8, background: saving ? "var(--c-text-4)" : "linear-gradient(135deg, #0066FF, #7C3AED)", color: "white", border: "none", fontSize: 13, fontWeight: 600, cursor: saving ? "wait" : "pointer", fontFamily: "inherit" }}
              >
